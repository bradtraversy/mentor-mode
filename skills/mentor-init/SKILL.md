...
1. Verify the pack is installed: `.claude/hooks/mentor-guard.mjs` exists and `.claude/settings.json` has the PreToolUse entry that runs it. If not, stop and tell the user to run the pack installer first (`npx create-mentor-mode@latest` in this repo).
